...
def analyze_tw_posts(csv_path, top_n_groups=5):
    """Top‑level entrypoint you can import and call from other code."""
    ...
    # 1) Overall stats
    print_overall(rows)
    # 2) Grouped stats (by source, then by source+lang if available)
    numeric_cols, _ = detect_column_types(rows)
    if "source" in rows[0]:
        print_group_means(rows, ["source"], numeric_cols, top_n_groups)
    if "source" in rows[0] and "lang" in rows[0]:
        print_group_means(rows, ["source", "lang"], numeric_cols, top_n_groups)

if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Analyze TW posts CSV")
    p.add_argument("csv_path", help="Path to the CSV file")
    p.add_argument("--top", type=int, default=5, help="How many groups to show")
    args = p.parse_args()
    analyze_tw_posts(args.csv_path, args.top)
